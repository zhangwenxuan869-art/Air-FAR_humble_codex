...
std::shared_ptr<tf2_ros::TransformBroadcaster> tfBroadcasterPointer;
...
void stateEstimationHandler(const nav_msgs::msg::Odometry::ConstSharedPtr& odom)
{
  ...
  geoQuat = makeQuaternionMsg(0, trackPitch, trackYaw);
  ...
  odomTrans.header.stamp = odom->header.stamp;
  odomTrans.header.frame_id = "map";
  odomTrans.child_frame_id = "track_point";
  odomTrans.transform.translation.x = trackX;
  odomTrans.transform.translation.y = trackY;
  odomTrans.transform.translation.z = trackZ;
...
int main(int argc, char** argv)
{
  ...
  nh = rclcpp::Node::make_shared("pathFollower");  declareAndGet("stateEstimationTopic", stateEstimationTopic);  declareAndGet("desiredTrajFile", desiredTrajFile);  declareAndGet("executedTrajFile", executedTrajFile);  declareAndGet("saveTrajectory", saveTrajectory);  declareAndGet("saveTrajInverval", saveTrajInverval);  declareAndGet("waypointTest", waypointTest);  declareAndGet("waypointNum", waypointNum);  declareAndGet("waypointInterval", waypointInterval);  declareAndGet("waypointYaw", waypointYaw);  declareAndGet("waypointZ", waypointZ);  declareAndGet("autonomyMode", autonomyMode);  declareAndGet("pubSkipNum", pubSkipNum);  declareAndGet("trackingCamBackward", trackingCamBackward);  declareAndGet("trackingCamXOffset", trackingCamXOffset);  declareAndGet("trackingCamYOffset", trackingCamYOffset);  declareAndGet("trackingCamZOffset", trackingCamZOffset);  declareAndGet("trackingCamScale", trackingCamScale);  declareAndGet("trackPitch", trackPitch);  declareAndGet("lookAheadScale", lookAheadScale);  declareAndGet("minLookAheadDis", minLookAheadDis);  declareAndGet("minSpeed", minSpeed);  declareAndGet("maxSpeed", maxSpeed);  declareAndGet("accXYGain", accXYGain);  declareAndGet("velXYGain", velXYGain);  declareAndGet("posZBoostScale", posZBoostScale);  declareAndGet("posXYGain", posXYGain);  declareAndGet("stopVelXYGain", stopVelXYGain);  declareAndGet("stopPosXYGain", stopPosXYGain);  declareAndGet("smoothIncrSpeed", smoothIncrSpeed);  declareAndGet("maxRollPitch", maxRollPitch);  declareAndGet("yawRateScale", yawRateScale);  declareAndGet("yawGain", yawGain);  declareAndGet("yawBoostScale", yawBoostScale);  declareAndGet("maxRateByYaw", maxRateByYaw);  declareAndGet("velZScale", velZScale);  declareAndGet("posZGain", posZGain);  declareAndGet("maxVelByPosZ", maxVelByPosZ);  declareAndGet("manualSpeedXY", manualSpeedXY);  declareAndGet("manualSpeedZ", manualSpeedZ);  declareAndGet("manualYawRate", manualYawRate);  declareAndGet("slowTurnRate", slowTurnRate);  declareAndGet("minSlowTurnCurv", minSlowTurnCurv);  declareAndGet("minSlowTurnInterval", minSlowTurnInterval);  declareAndGet("minStopRotInterval", minStopRotInterval);  declareAndGet("stopRotDelayTime", stopRotDelayTime);  declareAndGet("stopRotDis", stopRotDis);  declareAndGet("stopRotYaw1", stopRotYaw1);  declareAndGet("stopRotYaw2", stopRotYaw2);  declareAndGet("stopDis", stopDis);  declareAndGet("slowDis", slowDis);  declareAndGet("joyDeadband", joyDeadband);  declareAndGet("joyToSpeedDelay", joyToSpeedDelay);  declareAndGet("shiftGoalAtStart", shiftGoalAtStart);  declareAndGet("goalX", goalX);  declareAndGet("goalY", goalY);  declareAndGet("goalZ", goalZ);
  ...
  trackPath.poses[0].pose.position.z = goalZ;
  ...
  auto pubMarker = nh->create_publisher<visualization_msgs::msg::Marker>("/track_point_marker", 5);
  pubMarkerPointer = pubMarker;

  auto pubOdometry = nh->create_publisher<nav_msgs::msg::Odometry>("/track_point_odom", 5);
  pubOdometryPointer = pubOdometry;

  auto pubPath = nh->create_publisher<nav_msgs::msg::Path>("/track_path", 5);
  pubPathPointer = pubPath;

  auto pubControl = nh->create_publisher<geometry_msgs::msg::TwistStamped>("/attitude_control", 5);
  pubControlPointer = pubControl;

  auto pubAutoMode = nh->create_publisher<std_msgs::msg::Float32>("/auto_mode", 5);
  ...
  auto tfBroadcaster = std::make_shared<tf2_ros::TransformBroadcaster>(*nh);
