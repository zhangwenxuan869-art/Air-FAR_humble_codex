...
void DPUtil::ResetCloudIntensityWithTime(const PointCloudPtr& cloudInOut) {
  const float curTime = rclcpp::Clock().now().seconds() - DPUtil::systemStartTime;
                               ...
                               const PointCloudPtr& cloudInOut) 
{
  ...
  try {
    ...
    sensor_cloud.header.frame_id = from_frame_id;
    const auto transform_stamped = tf_buffer->lookupTransform(to_frame_id, from_frame_id, tf2::TimePointZero, std::chrono::seconds(1));
    tf2::doTransform(sensor_cloud, transformed_cloud, transform_stamped);
    pcl::fromROSMsg(transformed_cloud, *cloudInOut);
                                   ...
                                   Point3D& point)
{
  ...
  point_in.header.frame_id = from_frame_id;
  point_in.point = DPUtil::Point3DToGeoMsgPoint(point);
  try {
    const auto transform_stamped = tf_buffer->lookupTransform(to_frame_id, from_frame_id, tf2::TimePointZero, std::chrono::seconds(1));
...
geometry_msgs::msg::Point DPUtil::Point3DToGeoMsgPoint(const Point3D& point) {
