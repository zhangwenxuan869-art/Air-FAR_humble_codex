...
template <typename T>
static void DeclareAndGet(const rclcpp::Node::SharedPtr& nh,
...
void DPMaster::Init() {
  ...
  vertices_PCL_pub_ = nh_->create_publisher<sensor_msgs::msg::PointCloud2>("/vertics",5);
  obs_world_pub_ = nh_->create_publisher<sensor_msgs::msg::PointCloud2>("/dynamic_obs_world",1);
  new_PCL_pub_ = nh_->create_publisher<sensor_msgs::msg::PointCloud2>("/new_PCL",1);
  mapping_time_pub_ = nh_->create_publisher<std_msgs::msg::Float32>("/far_mapping_time", 5);

  //DEBUG Publisher
  dynamic_obs_pub_     = nh_->create_publisher<sensor_msgs::msg::PointCloud2>("/DP_dynamic_obs_debug",1);
  surround_free_debug_ = nh_->create_publisher<sensor_msgs::msg::PointCloud2>("/DP_free_debug",1);
  ground_pc_debug_ = nh_->create_publisher<sensor_msgs::msg::PointCloud2>("/DP_gnd_debug",1);
  surround_obs_debug_  = nh_->create_publisher<sensor_msgs::msg::PointCloud2>("/DP_obs_debug",1);
  scan_grid_debug_     = nh_->create_publisher<sensor_msgs::msg::PointCloud2>("/DP_scanGrid_debug",1);
...
void DPMaster::MainLoopCallBack() {
    ...
    if (graph_manager_.UpdateOdom(robot_pos_, cur_layer_idxs_, is_robot_stop_) || is_goal_update_ || is_dyobs_update_ || !is_graph_init_) {
      ...
      std_msgs::msg::Float32 mapping_timer;
...
void DPMaster::LoadROSParams() {
  ...
  // master params
  DeclareAndGet<float>(nh_, master_prefix + "main_run_freq", master_params_.main_run_freq, 5.0);
  DeclareAndGet<float>(nh_, master_prefix + "voxel_dim", master_params_.voxel_dim, 0.2);
  DeclareAndGet<float>(nh_, master_prefix + "robot_dim", master_params_.robot_dim, 0.5);
  ...
  DeclareAndGet<bool>(nh_, master_prefix  + "is_inter_navpoint", master_params_.is_inter_navpoint, true);
  DeclareAndGet<bool>(nh_, master_prefix  + "is_simulation", master_params_.is_simulation, true);
  DeclareAndGet<bool>(nh_, master_prefix  + "is_trajectory", master_params_.is_trajectory_edge, false);
  DeclareAndGet<bool>(nh_, master_prefix  + "attempt_autoswitch", master_params_.is_attempt_autoswitch, false);
  DeclareAndGet<std::string>(nh_, master_prefix + "world_frame", master_params_.world_frame, "map");

  // map handler params
  DeclareAndGet<float>(nh_, map_prefix + "ceil_length", map_params_.ceil_length, 5.0);
  DeclareAndGet<float>(nh_, map_prefix + "map_grid_max_length", map_params_.grid_max_length, 5000.0);
  DeclareAndGet<float>(nh_, map_prefix + "map_grad_max_height", map_params_.grid_max_height, 100.0);
  ...
  DeclareAndGet<int>(nh_, msger_prefix + "robot_id", msger_parmas_.robot_id, 0);
  msger_parmas_.frame_id = master_params_.world_frame;

  // graph planner params
  DeclareAndGet<float>(nh_, planner_prefix + "converge_distance", gp_params_.converge_dist, 1.0);
  DeclareAndGet<float>(nh_, planner_prefix + "goal_adjust_radius", gp_params_.adjust_radius, 10.0);
  DeclareAndGet<float>(nh_, planner_prefix + "goal_z_tolerance", gp_params_.goal_z_tolerance, 2.0);
  DeclareAndGet<int>(nh_, planner_prefix   + "free_counter_thred", gp_params_.free_thred, 5);
  DeclareAndGet<float>(nh_, planner_prefix + "free_box_dim", gp_params_.free_box_dim, 1.0);
  DeclareAndGet<int>(nh_, planner_prefix   + "reach_goal_vote_size", gp_params_.votes_size, 5);
  DeclareAndGet<float>(nh_, planner_prefix + "path_momentum_ratio", gp_params_.momentum_dist, 1.0);
  DeclareAndGet<int>(nh_, planner_prefix   + "path_momentum_thred", gp_params_.momentum_thred, 5);
  DeclareAndGet<int>(nh_, planner_prefix   + "clear_inflate_size", gp_params_.clear_inflate_size, 3);
  gp_params_.is_autoswitch = master_params_.is_attempt_autoswitch;

  // dynamic graph params
  DeclareAndGet<float>(nh_, graph_prefix  + "node_near_dist", graph_params_.near_dist, 2.0);
  DeclareAndGet<float>(nh_, graph_prefix  + "move_thred", graph_params_.move_thred, 0.25);
  DeclareAndGet<int>(nh_, graph_prefix    + "terrain_inflate_size", graph_params_.terrian_inflate, 2);
  DeclareAndGet<int>(nh_, graph_prefix    + "connect_votes_size", graph_params_.votes_size, 10);
  DeclareAndGet<int>(nh_, graph_prefix    + "clear_dumper_thred", graph_params_.dumper_thred, 3);
  DeclareAndGet<int>(nh_, graph_prefix    + "node_finalize_thred", graph_params_.finalize_thred, 3);
  DeclareAndGet<int>(nh_, graph_prefix    + "RANSAC_pool_size", graph_params_.pool_size, 12);
  DeclareAndGet<float>(nh_, graph_prefix  + "connect_angle_thred", graph_params_.kConnectAngleThred, 10.0);
  DeclareAndGet<float>(nh_, graph_prefix  + "trajectory_interval_ratio", graph_params_.traj_interval_ratio, 2.0);
  DeclareAndGet<float>(nh_, graph_prefix  + "pos_filter_margin", graph_params_.filter_pos_margin, 0.5);
  DeclareAndGet<float>(nh_, graph_prefix  + "dirs_filter_margin", graph_params_.filter_dirs_margin, 10.0);
  ...
  // utility params
  DeclareAndGet<float>(nh_, utility_prefix + "new_intensity_thred", DPUtil::kNewPIThred, 2.0);
  DeclareAndGet<float>(nh_, utility_prefix + "nav_clear_dist", DPUtil::kNavClearDist, 0.5);
  DeclareAndGet<float>(nh_, utility_prefix + "terrain_free_Z", DPUtil::kFreeZ, 0.1);
  DeclareAndGet<float>(nh_, utility_prefix + "dynamic_obs_dacay_time", DPUtil::kDecayTime, 10.0);
  DeclareAndGet<int>(nh_, utility_prefix   + "dyosb_update_thred", DPUtil::kDyObsThred, 4);
  DeclareAndGet<int>(nh_, utility_prefix   + "new_point_counter", DPUtil::KNewPointC, 10);
  DeclareAndGet<float>(nh_, utility_prefix + "angle_noise", DPUtil::kAngleNoise, 15.0);
  DeclareAndGet<float>(nh_, utility_prefix + "vertical_angle_thred", DPUtil::kVerticalAngleThred, 30.0);
  DeclareAndGet<float>(nh_, utility_prefix + "accept_max_align_angle", DPUtil::kAcceptAlign, 15.0);
  // convert to radians
  ...
  // scan handler params
  DeclareAndGet<int>(nh_, scan_prefix + "inflate_scan_size", scan_params_.inflate_size, 2);
  ...
  DeclareAndGet<bool>(nh_, cdetect_prefix        + "is_inflate_image", cdetect_params_.is_inflate, true);
  DeclareAndGet<bool>(nh_, cdetect_prefix        + "is_save_img", cdetect_params_.is_save_img, false);
  DeclareAndGet<float>(nh_, cdetect_prefix       + "resize_ratio", cdetect_params_.kRatio, 5.0);
  DeclareAndGet<double>(nh_, cdetect_prefix       + "approx_eps", cdetect_params_.approx_eps, 0.0005);
  DeclareAndGet<int>(nh_, cdetect_prefix         + "filter_count_value", cdetect_params_.kThredValue, 10);
  DeclareAndGet<int>(nh_, cdetect_prefix         + "img_blur_size", cdetect_params_.kBlurSize, 10);
  DeclareAndGet<std::string>(nh_, cdetect_prefix + "img_folder_path", cdetect_params_.img_path, "");
  ...
  DeclareAndGet<float>(nh_, contour_prefix + "around_distance", cg_params_.kAroundDist, 1.0);
  DeclareAndGet<float>(nh_, contour_prefix + "pillar_perimeter_dist", cg_params_.kPillarPerimeter, 4.0);
  DeclareAndGet<int>(nh_, contour_prefix + "KDTree_num", cg_params_.KD_TREE_K, 5);
  DeclareAndGet<int>(nh_, contour_prefix + "KDTree_neighbor_size", cg_params_.knn_search_num_, 1);
  DeclareAndGet<float>(nh_, contour_prefix + "KNN_search_radius", cg_params_.knn_search_radius_, 1.5);
  DeclareAndGet<float>(nh_, contour_prefix + "wall_insert_factor", cg_params_.wall_insert_factor, 0.6);
  cg_params_.voxel_dim = master_params_.voxel_dim;
  graph_params_.wall_insert_factor = cg_params_.wall_insert_factor;

}

void DPMaster::OdomCallBack(const nav_msgs::msg::Odometry::ConstSharedPtr msg) {
  ...
  if (!DPUtil::IsSameFrameID(odom_frame, master_params_.world_frame)) {
    ...
    try
    {
      auto transform_stamped = tf_buffer_->lookupTransform(master_params_.world_frame, odom_frame, tf2::TimePointZero, std::chrono::seconds(1));
  ...
  if (!is_odom_init_) {
    // system start time
    DPUtil::systemStartTime = nh_->now().seconds();
...
void DPMaster::PrcocessCloud(const sensor_msgs::msg::PointCloud2::ConstSharedPtr pc,
...
void DPMaster::ScanCallBack(const sensor_msgs::msg::PointCloud2::ConstSharedPtr scan_pc) {
...
void DPMaster::TerrainLocalCallBack(const sensor_msgs::msg::PointCloud2::ConstSharedPtr pc) {
...
void DPMaster::TerrainCallBack(const sensor_msgs::msg::PointCloud2::ConstSharedPtr pc) {
  if (!is_odom_init_) {
...
void DPMaster::GoalPoseCallBack(const geometry_msgs::msg::PoseStamped::ConstSharedPtr msg) {
