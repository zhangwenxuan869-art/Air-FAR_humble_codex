...
void TerrainPlanner::Init(const rclcpp::Node::SharedPtr nh, const TerrainPlannerParams& params) {
    ...
    terrian_map_pub_  = nh_->create_publisher<sensor_msgs::msg::PointCloud2>("/local_terrain_map_debug", 5);
}
...
void TerrainPlanner::VisualPaths() {
    ...
    auto DrawPath = [&](const PointStack& path) {
        ...
        geometry_msgs::msg::Point last_p = DPUtil::Point3DToGeoMsgPoint(path[0]);
