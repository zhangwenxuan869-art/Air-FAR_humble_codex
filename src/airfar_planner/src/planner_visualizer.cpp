...
void DPVisualizer::Init(const rclcpp::Node::SharedPtr nh) {
    ...
    // Rviz Publisher
    viz_node_pub_    = nh_->create_publisher<Marker>("/viz_node_topic", 5);
    viz_path_pub_    = nh_->create_publisher<Marker>("/viz_path_topic", 5);
    viz_graph_pub_   = nh_->create_publisher<MarkerArray>("/viz_graph_topic", 5);
    viz_contour_pub_ = nh_->create_publisher<MarkerArray>("/viz_contour_topic", 5);
    viz_map_pub_     = nh_->create_publisher<MarkerArray>("/viz_grid_map_topic", 5);
...
void DPVisualizer::VizGraph(const NodePtrStack& graph) {
    ...
    for (const auto& nav_node_ptr : graph) {
        ...
        const geometry_msgs::msg::Point cpoint = DPUtil::Point3DToGeoMsgPoint(nav_node_ptr->position);
        ...
        //             const geometry_msgs::msg::Point cpoint_tmp = DPUtil::Point3DToGeoMsgPoint(cnode->position);
                               ...
                               const float& ceil_length, const float& ceil_height)
{
    ...
    for (std::size_t i=0; i<N1; i++) {
        geometry_msgs::msg::Point p = DPUtil::Point3DToGeoMsgPoint(neighbor_centers[i]);
        neighbor_marker.points[i] = p;
    }
    for (std::size_t i=0; i<N2; i++) {
        geometry_msgs::msg::Point p = DPUtil::Point3DToGeoMsgPoint(occupancy_centers[i]);
